...
ap.add_argument("-id", "--id", required=False, default=None, help="Document ID to fetch (either id or url must be provided)")
ap.add_argument("-url", "--url", required=False, default=None, help="Document URL to fetch (either id or url must be provided)")
...
if not args['id'] and not args['url']:
    ap.error("At least one of --id or --url must be provided")

fetcher = create_collection_fetcher(collection_name=args['collection'])
result = fetcher.fetch(id=args['id'], url=args['url'], start_line=args['startLine'], end_line=args['endLine'])
