...
def create_collection_searcher(collection_name, index_name, filter=None):
    return log_execution_duration(
        lambda: __create_collection_searcher(collection_name, index_name, filter),
        identifier=f"Preparing collection searcher"
    )

def __create_collection_searcher(collection_name, index_name, filter):
