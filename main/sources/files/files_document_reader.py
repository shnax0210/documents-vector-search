    ...
    def __read_file_pathes(self):
        ...
        for root, _, files in os.walk(self.base_path):
            for file_name in files:
                rel_path = os.path.relpath(os.path.join(root, file_name), self.base_path)
                if (
                    os.path.isfile(os.path.join(root, file_name))
                    and self.__is_file_included(rel_path)
                    and not any(rel_path.endswith(ext) for ext in EXCLUDED_FILE_EXTENSIONS)
                    and not self.__is_file_excluded(rel_path)
                ):
